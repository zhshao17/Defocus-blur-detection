...
def train(epo_num=1):
    ...
    print(device)
    enet_model = enet.ENet(num_classes=2)
    enet_model = enet_model.to(device)
    # 这里只有两类，采用二分类常用的损失函数BCE
    criterion = nn.BCELoss().to(device)
    # 随机梯度下降优化，学习率0.001，惯性分数0.7
    optimizer = optim.Adam(enet_model.parameters(), lr=1e-3)
    ...
    for epo in range(epo_num):
        ...
        if np.mod(epo, 10) == 0:
            # 只存储模型参数
            torch.save(enet_model.state_dict(), './enet_model_{}.pth'.format(epo))
            stict = './enet_model_{}.pth'.format(epo)
            print(stict)
            print('saveing ./enet_model_{}.pth'.format(epo))

        if np.mod(epo, 10) == 0:

            test.test(stict,enet_model, "./models_result", test_dataloader)
