...
class BR2Net(nn.Module):
    ...
    def forward(self, x):
        ...
        predictL2H1 = self.predictL2H1(
            torch.cat((predictL2H0, F.upsample(CAlayer1, size=l0_size, mode='bilinear')), 1)) + predictL2H0
        predictL2H2 = self.predictL2H2(
            torch.cat((predictL2H1, F.upsample(CAlayer2, size=l0_size, mode='bilinear')), 1)) + predictL2H1
        predictL2H3 = self.predictL2H3(
            torch.cat((predictL2H2, F.upsample(CAlayer3, size=l0_size, mode='bilinear')), 1)) + predictL2H2
        predictL2H4 = self.predictL2H4(
            torch.cat((predictL2H3, F.upsample(CAlayer4, size=l0_size, mode='bilinear')), 1)) + predictL2H3
        predictH2L0 = self.predictH2L0(CAlayer4)
        predictH2L1 = self.predictH2L1(
            torch.cat((predictH2L0, F.upsample(CAlayer3, size=l4_size, mode='bilinear')), 1)) + predictH2L0
        predictH2L2 = self.predictH2L2(
            torch.cat((predictH2L1, F.upsample(CAlayer2, size=l4_size, mode='bilinear')), 1)) + predictH2L1
        predictH2L3 = self.predictH2L3(
            torch.cat((predictH2L2, F.upsample(CAlayer1, size=l4_size, mode='bilinear')), 1)) + predictH2L2
        predictH2L4 = self.predictH2L4(
            torch.cat((predictH2L3, F.upsample(CAlayer0, size=l4_size, mode='bilinear')), 1)) + predictH2L3
        ...
        if self.training:
            return self.Sigmoid(predictL2H0), self.Sigmoid(predictL2H1), self.Sigmoid(predictL2H2), self.Sigmoid(predictL2H3), self.Sigmoid(predictL2H4),\
                    self.Sigmoid(predictH2L0), self.Sigmoid(predictH2L1), self.Sigmoid(predictH2L2), self.Sigmoid(predictH2L3), self.Sigmoid(predictH2L4),\
