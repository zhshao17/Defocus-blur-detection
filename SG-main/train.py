...
j = 0
# 开始训练
pbar = range(args.epoch_start, args.epoch_num)
save_step = 10
for i in pbar:
    print('epoch=' + str(i + 1))
    try:
    ...
    patch = (1, 1, 1)
    valid = Variable(torch.FloatTensor(np.ones((real_image.size(0), *patch))), requires_grad=False)
    fake = Variable(torch.FloatTensor(np.zeros((real_image.size(0), *patch))), requires_grad=False)
    # 不用显卡运行
    ...
    if args.checkpoint_interval != -1 and i % save_step == 0:
        ...
        torch.save(generator.state_dict(),
                   'log/%s-%s/%s/generator_%d.pth' % (args.exp_name, args.dataset_name, args.model_result_dir, i))
        torch.save(discriminator.state_dict(),
                   'log/%s-%s/%s/discriminator1_%d.pth' % (args.exp_name, args.dataset_name, args.model_result_dir, i))
        torch.save(discriminator2.state_dict(),
                   'log/%s-%s/%s/discriminator2_%d.pth' % (args.exp_name, args.dataset_name, args.model_result_dir, i))
